...
async function requireAdmin() {
  ...
  if (!user) {
    return { error: NextResponse.json({ error: '인증이 필요합니다.' }, { status: 401 }), supabase: null, user: null }
  }

  const role = await getUserRole(user.id)
  if (role !== 'administrator') {
    return { error: NextResponse.json({ error: '관리자 권한이 필요합니다.' }, { status: 403 }), supabase: null, user: null }
  }
