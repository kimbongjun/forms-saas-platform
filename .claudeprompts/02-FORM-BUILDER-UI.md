...
## 빌더 레이아웃 (EditFormBuilder)

```
┌─ Header ─────────────────────────────────────────────────┐
│  ← 뒤로   [BuilderTabBar: 편집 / 설정 / 응답]  [저장]   │
├─ [편집 탭] ──────────────────────────────────────────────┤
│  BuilderSidebar (w-56)   │  BuilderCanvas (flex-1)        │
│  [INPUT_TYPES]           │  FieldCard × N (DnD 순서변경)  │
│  · 텍스트, 이메일 등      │                               │
│  [CONTENT_TYPES]         │                               │
│  · HTML, 이미지, 지도 등  │                               │
├─ [설정 탭] ──────────────────────────────────────────────┤
│  SettingsPanel (전체 폭)                                  │
│  · 배너/썸네일, 테마 컬러, 알림·마감·웹훅, 이메일 템플릿  │
│  · 다국어 설정, SEO 옵션                                  │
├─ [응답 탭] (EditFormBuilder 전용) ───────────────────────┤
│  ResponsesTab: 최근 응답 + 통계 요약 + CSV 링크           │
└──────────────────────────────────────────────────────────┘
```
